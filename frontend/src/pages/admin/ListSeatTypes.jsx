...
const ListSeatTypes = () => {
  ...
  const fetchSeatTypes = async () => {
    try {
      ...
      toast.error("Lỗi khi lấy danh sách loại ghế");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setSeatTypes(seatTypes.filter((st) => st._id !== id));
      } else {
        toast.error(data.message);
      }
    } catch (error) {
  ...
  return isLoading ? (
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Quản lý loại ghế</h1>
        <button
          onClick={() => navigate("/admin/seat-types/create")}
      ...
      {filteredSeatTypes.length === 0 ? (
        <div className="text-center py-20 text-gray-400">
          <p>
            {searchTerm
              ? "Không tìm thấy loại ghế phù hợp"
          ...
          {filteredSeatTypes.map((seatType) => (
            ...
            >
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold mb-1">{seatType.name}</h3>
              ...
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => navigate(`/admin/seat-types/${seatType._id}`)}
                ...
                <button
                  onClick={() =>
                    navigate(`/admin/seat-types/${seatType._id}/edit`)
                  }
                ...
                <button
                  onClick={() => handleDelete(seatType._id)}
